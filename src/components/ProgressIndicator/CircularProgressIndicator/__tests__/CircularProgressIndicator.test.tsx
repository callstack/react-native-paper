...
it('renders determinate progress', () => {
  const tree = render(<CircularProgressIndicator progress={0.3} />).toJSON();
...
it('renders the active stroke and no track when indeterminate', () => {
  const tree = render(<CircularProgressIndicator indeterminate testID="pi" />);
...
it('reports the determinate value', () => {
  const tree = render(<CircularProgressIndicator progress={0.42} />);
...
it('is busy below 100% and not busy at 100% (determinate)', () => {
  const partial = render(<CircularProgressIndicator progress={0.5} />);
  expect(partial.getByRole(a11yRole).props.accessibilityState).toEqual({
    busy: true,
  });

  const complete = render(<CircularProgressIndicator progress={1} />);
...
it('is busy when indeterminate', () => {
  const tree = render(<CircularProgressIndicator indeterminate />);
...
it('shows the track again when toggling indeterminate -> determinate', () => {
  ...
  tree.rerender(<CircularProgressIndicator progress={0.3} testID="pi" />);
