...
class TextInput extends React.Component<Props, State> {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.value !== parentState.value ||
      prevState.focused !== parentState.focused ||
      prevProps.placeholder !== this.props.placeholder
    ) {
      if (parentState.value || parentState.focused) {
        this._minmizeLabel();
      } else {
        this._restoreLabel();
      }
    ...
    if (
      ...
      if (this.props.error || parentState.focused) {
        this._showColor();
      } else {
        this._hideColor();
      }
  ...
  _showError = () => {
    Animated.timing(this.props.parentState.error, {
      toValue: 1,
  ...
  _restoreLabel = () =>
    Animated.timing(this.props.parentState.labeled, {
      toValue: 1,
      duration: FOCUS_ANIMATION_DURATION,
    }).start();
  ...
  render() {
    ...
    const labelTranslateY = parentState.labeled.interpolate({
      inputRange: [0, 1],
          ...
          <Animated.View
            style={[
              ...
              this._getBottomLineStyle(
                errorColor,
                // $FlowFixMe
                Animated.multiply(this.state.underline, parentState.error)
              ),
            ]}
          />
        </View>
