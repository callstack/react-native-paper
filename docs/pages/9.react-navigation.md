...
We can easily add additional actions to the header thanks to the `Appbar.Action` component provided by Paper. For example, we can add menu button and render it only on the main screen.
```js
function CustomNavigationBar({navigation, previous}) {
  return (
    <Appbar.Header>
