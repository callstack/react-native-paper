...
`Card` component's property `mode` has been expanded with one additional option called `filled`, which applies to the card specified background color, without any elevation and border.
...
Since there is no one right way to make a card, there is also o one right way for specifing title and subtitle variant, therefore two new props come in handy:
