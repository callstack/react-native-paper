...
Paper now supports both Material Design 2 and 3 through the configuration described in “Versioning” and is compatible with a handful of API change 
...
`Card` component received new prop: `filled`. 

Now there are three of types cards :
* `elevated` - with default elevation 
* `filled` - with backgroundColor, no elevation and border
* `outlined` - with border and no elevation
...
`Card.Actions` are on the right side and the bottom corners of the images are rounded. 

According to the updates in Typography `<Title>` and <`Caption>` has been replaced by `<Text>` with proper variants.
