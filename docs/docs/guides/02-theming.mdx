...
To support custom themes, paper exports a `ThemeProvider` component. You need to wrap your root component with the provider to be able to support themes:
...
By default React Native Paper will apply the [Material You theme (MD3)](https://github.com/callstack/react-native-paper/blob/main/src/styles/themes/v3/LightTheme.tsx) if no `theme` or `version` prop is passed to to the `ThemeProvider`.
...
import { ThemeProvider, MD3LightTheme, adaptNavigationTheme } from 'react-native-paper';
...
1. Pass your theme overrides to the ThemeProvider component
...
In order to use the Material Design 2 theme you can just pass `{ version: 2 }` to the ThemeProvider theme prop:
...
```js
import * as React from 'react';
import { MD2LightTheme, ThemeProvider } from 'react-native-paper';
import App from './src/App';
...
If you want to change the theme for a certain component from the library, you can directly pass the `theme` prop to the component. The theme passed as the prop is merged with the theme from the `ThemeProvider`:
...
The `ThemeProvider` exposes the theme to the components via [React's context API](https://reactjs.org/docs/context.html), which means that the component must be in the same tree as the `ThemeProvider`. Some React Native components will render a different tree such as a `Modal`, in which case the components inside the `Modal` won't be able to access the theme. The work around is to get the theme using the `withTheme` HOC and pass it down to the components as props, or expose it again with the exported `ThemeProvider` component.
